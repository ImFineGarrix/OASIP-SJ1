...
//    @Autowired JwtTokenUtil jwtUtil;
    ...
    private boolean getResponseEntity(@RequestBody LoginFormDTO user) {
        ...
        if(user.getEmail() != null && checkMessage.isEmpty()){
            ...
            if(us == null){
                checkMessage.put("email-or-password-invalid","email or password is not valid.");
            }
        }
        ...
        if(user.getPassword() != null && repository.findByEmail(user.getEmail().trim()) != null && checkMessage.isEmpty()){
            if(!encoder.matches(user.getPassword(),repository.findByEmail(user.getEmail()).getPassword()) && checkMessage.get("email-or-password") == null){
                checkMessage.put("email-or-password-invalid","email or password is not valid.");
            }
        }
    ...
    public ResponseEntity match(LoginFormDTO user, HttpServletRequest request) throws Exception{
        if(getResponseEntity(user)){
            ...
            return ResponseEntity.ok(new AuthResponse(user.getEmail(),token));
             ...
             if(checkMessage.get("email-or-password-invalid") != null){
